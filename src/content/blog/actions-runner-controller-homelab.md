---
title: Self-hosting GitHub Actions runners when your tests outgrow the free tier
author: aj
date: 2026-06-01
draft: true
...
The e2e job kept hitting its 20-minute timeout on `ubuntu-latest`. The failure was not in a test; the runner just ran out of clock. Looking at the logs, most of the budget was going to provisioning, not to the tests themselves:

- `pnpm install` for a Vue project with a long-tail dep graph.
...
The cache helped, but cache hits are not guaranteed across PRs, and a cache miss on the Playwright browsers ate the entire timeout budget. I could have bumped `timeout-minutes` to 30 or 40 and stopped looking, but at that point the bottleneck was clearly the runner image, not the tests. Free-tier `ubuntu-latest` runners are general-purpose; they have no idea I want a browser test environment.

What I wanted was a runner where the browsers and matching system libraries were already on disk. Playwright publishes [exactly that image][6]: `mcr.microsoft.com/playwright`, tagged per Playwright version. I just had nowhere to run it as a GitHub Actions runner.

## Actions Runner Controller

[Actions Runner Controller][4] (ARC) is GitHub's official Kubernetes operator for self-hosted runners. The model is two Helm charts working together:
...
I deploy both charts through [Argo CD][7] with the multi-source pattern, the same way I run [CloudNativePG][8] and the rest of the homelab:
...
- `actions/setup-node@v5` - the Playwright image already has Node 24.
...
The failure sequence I walked through, just so it is in one place:
...
That is the situation I am in for my personal repos, so this setup is staying. If I picked up another repo with the same pattern, onboarding is now mechanical:
...
Same controller, same chart, same secret pattern. The architecture finally pays off on the second runner pool, not the first.
