...
The visual scores and the engineering scores agreed at the top and disagreed quite a bit in the middle. In this post I will share the results and what I learned from evaluating everything blind.

## The assignment

The prompt asked for a production-quality terminal application with a distinctive LCARS interface: asymmetric framing, sweeping rails, elbow and pill shapes approximated with terminal glyphs, bold color fields, and deliberate negative space. I told the models explicitly that I did not want `htop` with an orange theme.
...
Every submission was copied into a clean workspace with build caches, prebuilt binaries, and the model's own supplied captures excluded. Excluding the captures is important because a model that ships a flattering screenshot of its own work should not get to use that screenshot as evidence.
...
The screenshots in this post are fresh captures. I rebuilt all eight from source and re-captured them at 120×36 while writing this. The hostname is the only thing I edited, everything else is my actual machine.
...
The blind visual leader and the overall winner turned out to be the same submission, and the same is true for last place. In between, the engineering rubric moved things around quite a bit. The biggest move was Kimi K3, which I had ranked third on looks and which finished sixth.

All eight built cleanly on both platforms. All eight passed their own test and lint suites, with one exception noted below. None of these submissions were a disaster. The spread is between good and very good, and it comes down to the parts a screenshot cannot show.
...
This was the best structural translation of LCARS in the field, and I recorded that before knowing which model made it. It has segmented elbows, exposed rounded terminals, squared continuations, inset cutouts, and metrics tucked into the frame around a genuinely open working field. It is the only submission where the composition would still look like LCARS with all of its text blanked out.
...
The single deduction was a metrics point: the sampled macOS diagnostic frame gave thin evidence for per-process CPU. It was a small deduction and it did not threaten first place.
...
Only a point and a half behind first place, this was the most balanced submission of the eight. It has an excellent dominant clock, a strong asymmetric rail stack, a confident palette, and real negative space. During the blind evaluation I marked its central field as less structurally interlocked than `V07`'s.

Everything else is close to flawless: Go with a custom renderer, cleanly isolated native collectors, deterministic sampling and render tests, and complete documentation. Normal quit, <kbd>CTRL</kbd> + <kbd>C</kbd>, and `SIGTERM` all restored cursor, alternate screen, and termios. It also recorded the lowest steady-state terminal traffic of the leading entries at 703 bytes per second.
...
This submission had the strongest engineering relative to its visual score. It is vendored, offline-oriented Go with separate Darwin and Linux collectors, excellent fallbacks, its own capture utility, thorough tests, and documentation good enough that I used it as the reference for what the deliverables were supposed to look like. Redirected output fails safely with an actionable message rather than a panic.

The visual score is what holds it at third. During the blind evaluation I wrote that several of its controls remain flat-ended strips: the left column reads as a stack of labeled rectangles rather than the compound elbows and rounded terminals that give LCARS its shape language. It has a clear focal clock and deliberate segmentation but flatter geometry overall. 3.13 ms startup, 0.20% idle CPU, 9,928 KiB RSS.
...
This submission is compact, fast, and polished. It has strong hierarchy and a sweeping left elbow, but during the blind evaluation I noted that the elbow reads as one large continuous slab, and the segmentation rhythm that makes LCARS feel like a control surface is mostly absent. It is a clean, restrained interpretation rather than an ambitious one.
...
This one has deep compound framing and good segmentation, but during the blind evaluation I marked the hierarchy as indecisive and the overall result as somewhat box-like. It has the right LCARS pieces without quite pulling the composition together.
...
This is the submission where the blind visual test and the engineering evaluation disagreed the most, and it is a good example of why I did the two halves separately.

During the blind evaluation I scored it 31.5, ahead of Opus 4.8 on shape language in places. It has a bold silhouette, a circular CPU dial as a focal counterpoint to the clock, a staircase rail down the left, and the best sub-cell glyph work of the eight. If the evaluation had stopped at the screenshots, it would have placed third.
...
There was also a smaller honesty problem: `cargo fmt --check` failed on the delivered source despite a README claiming clean formatting, and the README claimed 37 tests but only 36 ran.
...
This is a coherent segmented command deck with excellent breathing room. It leans toward horizontal telemetry rows and uses less compound elbow shaping than the leaders. In the blind evaluation it tied GPT-5.6 Sol at 30.0.
...
This submission was last in the blind visual scores and last overall, and both verdicts point at the same thing. The clock is memorable and the negative space is real, but during the blind evaluation I wrote that most of the composition is a single broad left slab plus detached right-side decorations, with metrics that feel placed rather than integrated. It scored 9.5/15 on LCARS fidelity.
...
Pay attention to the output column. Kimi K3 writes nearly 30 times as much data to the terminal per second as Opus 5 for a comparable scene. Over SSH on a slow link, that is the difference between a smooth display and a smeared one.

I only have API cost data for two submissions. Grok 4.5 reported $1.07 across 88,664 tokens and Kimi K3 reported $4.76 across 135,036 tokens.
...
**It removed self-reported evidence.** Excluding every model's own supplied captures and rebuilding from clean snapshots turned out to matter more than the anonymization. Two submissions made README claims that did not survive being run. Treat a model's screenshot of its own output like marketing.

There are still limitations to this evaluation. Performance is a point-in-time measurement on one Linux host, not a benchmark sweep. Collectors were checked for plausibility against native tools on both platforms but I did not fault-inject every permission and counter-rollover path. And the visual half, blind or not, is still my personal taste. A blind test only removes the identity bias.

## Conclusion

In the end there was a 17.5-point spread between the best and worst of the eight models. The top four are all genuinely good programs I would run. The bottom four are also good programs, each with one specific thing wrong that a screenshot would never have told me: a hung <kbd>CTRL</kbd> + <kbd>C</kbd>, a missed `SIGTERM`, a panic on a redirected pipe, or an idle loop burning 6.5% of a core to draw a mostly static screen.
