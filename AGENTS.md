...
A personal blog. Astro 6 + TypeScript, static content collections under `src/content/blog/`, built to `dist/`. CI runs on GitLab; PRs/MRs land on `main`.

**Production is AWS Amplify** (S3 + CloudFront), built from `amplify.yml`. The nginx/Docker image published to the GitLab Container Registry is for local and test hosting — **`nginx.conf` has no effect on production.** Redirects and 404 handling for real users are Amplify app-level rules, which Amplify keeps outside the repo; the intended set is committed to `amplify-redirects.json`. Change the two together, or test hosting and production will disagree about routing. See `docs/deployment.md`.
...
- **Two typefaces, split by job.** DM Mono (`--font-mono`) is the chrome: header, banner, page titles on list/index pages, metadata, tags, TOC, tables, code blocks and their copy button, and the amber `##` heading-level markers. Literata (`--font-read`, `@fontsource-variable/literata`) is the reading layer: the post title and everything in `.prose` except code, tables, captions and `kbd` — paragraphs, lists, blockquotes, `h2`–`h4`. Outside posts, prose paragraphs opt in with `p.log.read` (the about page) and the page passes `readingFont` to `BaseLayout` so Literata is preloaded. Long passages in monospace were tiring to read; use the serif for sentences people read, not for chrome, lists of facts or data. Anything inside `.prose` that uses `font: inherit` (e.g. CopyCode's button) picks up the serif unless its container is pinned back to `--font-mono`. Both are preloaded in `BaseLayout.astro` (Literata only on posts and `readingFont` pages). DM Mono ligatures are off (`font-feature-settings: 'liga' 0`) so arrows in code render as literal `->` rather than the `→` glyph.
...
- **Every log line tells the truth.** Lean into the terminal theme on the chrome and index pages, but every device must report a real fact, computed at build time: boot lines on home count real posts/words/tags; `[ OK ]`/`[NOTE]`/`[WARN]` on posts reflect word count, the `updated` date, and whether the post is over two years stale; timestamps (`src/utils/log-clock.ts`) are real elapsed render time since the page's Banner started the clock; `●`/`○` unit markers come from `src/utils/post-state.ts`; `-- Reboot: YYYY --` separators count real entries; the `top` header on /tags and `df` table on /categories are real figures (their tooltips say what the numbers mean). Don't add a status line, count, or timestamp that isn't derived from data. The one deliberate exception is the 404 kernel panic, which is an obvious joke. Keep the reading layer (post body, about paragraphs) calm. Still avoid all-caps labels, `→` prefixes on links, and bordered tag chips (tags render as plain `#slug`).
- **Every text token passes WCAG AA.** `--fg-faint`, `--amber`, `--warn` etc. all clear 4.5:1 on `--bg`, `--bg-deep` and `--bg-card` in both themes — the 3:1 threshold only applies to large text and non-text UI, and status tags like `[INFO]` are 12px text. Light `--amber` is `#985a00` (4.6:1 on the darkest light surface); `.prose a` keeps the stronger `--amber-strong` (`#8a5300`) for body links. If you change a token, re-check it against all three surfaces in both themes. Interactive targets are at least 24px tall in the header and on touch screens (WCAG 2.5.8); a skip link precedes the header.
...
- `BaseLayout.astro` header — a tmux status line: session, window list (`0:home 1:posts* …`, current window flagged `*`), then the reading gauge on posts (`[ OK ] reached end of post` at the bottom), the build clock in tmux's `status-right` format, and the theme toggle (`:set bg=dark`). Deliberately quiet: `--bg-deep` with a hairline rule and faint text, green only on the session name. Not tmux's default solid green bar, which read as the loudest thing on the page. It stays one line at every width by shedding parts as it narrows (the window list scrolls without a visible scrollbar, so anything that squeezes it hides nav): the clock goes at ≤1023px, the session name and text gauge at ≤800px (the gauge becomes a 2px amber line along the header's bottom edge), and window numbers and the toggle's `:set` at ≤640px. If you add anything to the bar, re-check that the window list fits from 320px to 1440px.
- `Banner.astro` — boot banner at the top of every page. Props: `command`, `lines` (extra log lines), `status`, `showPreamble`. On home, rows stream in once per session (`html.booting`, set in BaseLayout's head script; off under reduced motion).
- `Module.astro` — section divider `[ts] ── name (count) ──────`. Props: `name`, optional `count` (a real figure), optional `id`.
...
- `PostCard.astro` — `systemctl status`-style `.unit` block, used on home: `●` state marker (green new <30d, amber updated <30d, grey `○` older), `Active:`, `CGroup:` (categories), `Tags:`.
- `JournalList.astro` — journalctl list for `/posts` and tag/category pages: `-- Journal begins … --` header and `-- Reboot: YYYY (n entries) --` year separators. Needs a `journal` summary from `journalSummary()` in `getStaticPaths` props.
...
- Posts use root-relative links (`/posts/foo/`, `/images/foo.png`, `/tags/foo/`). Lychee verifies these against `dist/`. **Keep the trailing slash** — production 301s `/posts/foo` to `/posts/foo/`, so omitting it costs a redirect hop on every visit.
- **Never build a taxonomy URL from the raw frontmatter term.** Always go through `slugify()` in `src/utils/slug.ts`. The term `Software Development` lives in frontmatter but its URL is `/categories/software-development/`. This reproduces Hugo's pre-migration slug rules; bypassing it re-breaks the URLs Google has indexed and reintroduces `%20` in paths. Terms differing only by case or separator (`macOS`/`macos`) intentionally collapse to one route — `groupBySlug()` in `src/utils/taxonomy.ts` aggregates their posts and picks the most-used spelling as the display label.
